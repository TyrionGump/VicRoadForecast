...
class RoadNetwork:
    ...
    def __init__(self, link_geo_path, lga_geo_path, poi_geo_path, processed_link_path=""):
        ...
        self._count_poi_around_link(buffer_distance=[100, 200, 300, 400, 500])
    ...
    def _count_poi_around_link(self, buffer_distance=[400]):
        ...
        for dist in buffer_distance:
            research_buffer_gdf['geometry'] = self.link_gdf.to_crs(epsg=3857).buffer(distance=dist)
            buffer_stats = gpd.sjoin(research_buffer_gdf, self.poi_gdf.to_crs(epsg=3857), how='left', op='contains')
            self.link_gdf['poi_num_{}m'.format(dist)] = list(buffer_stats.groupby('id').size())
