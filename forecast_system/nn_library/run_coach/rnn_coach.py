...
class RNN_COACH:
    ...
    def train(self, dataset: TorchDataset):
        ...
        for run in self.run_builder.get_runs(params=self.params):
            ...
            network = RNN(feature_size=dataset.feature_size, target_size=dataset.target_size,
                           hidden_size=run.hidden_size, num_layers=run.num_layers).to(run.device)
    ...
    def pred(self, dataset: TorchDataset, model_params: dict):
        network = RNN(feature_size=model_params['feature_size'], target_size=model_params['target_size'],
    ...
    def run(self):
        if args.separate:
            os.makedirs('result/rnn/separate/net_params', exist_ok=True)
            os.makedirs('result/rnn/separate/prediction_values', exist_ok=True)
            os.makedirs('result/rnn/separate/true_values', exist_ok=True)
            os.makedirs('result/rnn/separate/performance_results', exist_ok=True)
            os.makedirs('result/rnn/separate/training_log', exist_ok=True)

            for link_id in self.link_ids:
                logging.info('Running RNN for link {}'.format(link_id))
                ...
                torch.save(model_params, 'result/rnn/separate/net_params/{}.pth'.format(link_id))
                ...
                y_pred.to_csv('result/rnn/separate/prediction_values/{}.csv'.format(link_id), index=False)
                y_true.to_csv('result/rnn/separate/true_values/{}.csv'.format(link_id), index=False)
                performance.to_csv('result/rnn/separate/performance_results/{}.csv'.format(link_id), index=False)
                run_log.to_csv('result/rnn/separate/training_log/{}.csv'.format(link_id), index=False)

        else:
            os.makedirs('result/rnn/global/net_params', exist_ok=True)
            os.makedirs('result/rnn/global/prediction_values', exist_ok=True)
            os.makedirs('result/rnn/global/true_values', exist_ok=True)
            os.makedirs('result/rnn/global/performance_results', exist_ok=True)
            os.makedirs('result/rnn/global/training_log', exist_ok=True)
            ...
            model_params = self.train(training_torch_set)
            torch.save(model_params, 'result/rnn/global/net_params/global.pth')
            run_log = self.run_manager.get_run_log()
            ...
            for link_id in self.link_ids:
                ...
                y_pred = pd.DataFrame(y_pred)
                y_true = pd.DataFrame(y_true)
                y_pred.to_csv('result/rnn/global/prediction_values/{}.csv'.format(link_id), index=False)
                y_true.to_csv('result/rnn/global/true_values/{}.csv'.format(link_id), index=False)
                performance.to_csv('result/rnn/global/performance_results/{}.csv'.format(link_id), index=False)

            run_log.to_csv('result/rnn/global/training_log/global.csv', index=False)
