...
class LSTM(nn.Module):
    ...
    def _init_net_layers(self):
        ...
        self.dense = nn.Linear(in_features=self.hidden_size * 2, out_features=self.target_size)

    def forward(self, x):
        # out: [batch_size, sequence_num, hidden_size]
        # hidden_prev: [layers_num, batch_size, hidden_size]
        output, (hidden_prev, cell) = self.lstm(x)
        hidden_prev = self.dropout(torch.cat([hidden_prev[-2], hidden_prev[-1]], dim=1))
        out = self.dense(hidden_prev)
        # print(out.shape)
        return out
