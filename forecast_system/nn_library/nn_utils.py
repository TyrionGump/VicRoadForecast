...
class RunManager:
    ...
    def begin_epoch(self):
        ...
        self.epoch_mse = torch.zeros(args.research_config['window']['BACKWARD_STEPS']).to(self.run_params.device)
        self.epoch_mape = torch.zeros(args.research_config['window']['BACKWARD_STEPS']).to(self.run_params.device)
        self.epoch_mae = torch.zeros(args.research_config['window']['BACKWARD_STEPS']).to(self.run_params.device)
    ...
    def save(self):
        print(pd.DataFrame.from_dict(self.run_data, orient='columns'))
