...
class Dataset:
    ...
    def get_training_set(self):
        return self.training_feature_dict, self.training_target_dict

    def get_testing_set(self):
        return self.testing_feature_dict, self.testing_target_dict
    ...
    def aggregate_time_series(self, minute=5):
        ...
        print(self.training_data_dict[2727])
    ...
    def normalize(self):
        if self.separate:
            ...
            self.normalize_encoder = Normalizer()
...
class NN_Dataset(Dataset):
    def __init__(self, training_data_dict, testing_data_dict):
        super().__init__(training_data_dict, testing_data_dict, False)
        self.training_tensor_set = None
        self.testing_tensor_set = None

    def reshape_for_NN(self, forward_steps=36, backward_steps=12):
        ...
        for link_id in self.link_ids:
            ...
            training_feature_all.append(self.training_feature_dict[link_id])
            training_target_all.append(self.training_target_dict[link_id])
            testing_feature_all.append(self.testing_feature_dict[link_id])
            testing_target_all.append(self.testing_target_dict[link_id])

        self.training_tensor_set = TorchDataset(np.concatenate(training_feature_all, axis=0), np.concatenate(training_target_all, axis=0))
        self.testing_tensor_set = TorchDataset(np.concatenate(testing_feature_all, axis=0), np.concatenate(testing_target_all, axis=0))

    def get_training_set(self):
        return self.training_tensor_set

    def get_testing_set(self):
        return self.testing_tensor_set
    ...
    def _rolling_time_series(self, df: pd.DataFrame, forward_steps, backward_steps):
        ...
        # target = target * math.sqrt(self.normalize_encoder.var_[0]) + self.normalize_encoder.mean_[0]
